    ...
    private function setChildren(xml : Xml) {
        for (c in xml.elements()) {
            switch (c.nodeName) {
                ...
                case "properties":
                case "objectgroup":
                    var objectgroup = new TiledObjectGroup(c);
                    objects.set(objectgroup.name, objectgroup);
                case "imagelayer":
                case "group":
            }
        }
